    ...
    public partial class TilemapEditor : Editor
    {
        ...
        private void DoMapSceneGUI()
        {
            if (m_toggleMapBoundsEdit)
            {
                ...
                var fmh_1039_178_638985770347387054 = Quaternion.identity; Vector3 vMinX = Handles.FreeMoveHandle(m_tilemap.transform.TransformPoint(new Vector2(m_tilemap.MinGridX * m_tilemap.CellSize.x, m_tilemap.MapBounds.center.y)), 0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero, EditorCompatibilityUtils.CubeCap);
                var fmh_1040_185_638985770347410437 = Quaternion.identity; Vector3 vMaxX = Handles.FreeMoveHandle(m_tilemap.transform.TransformPoint(new Vector2((m_tilemap.MaxGridX + 1f) * m_tilemap.CellSize.x, m_tilemap.MapBounds.center.y)), 0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero, EditorCompatibilityUtils.CubeCap);
                var fmh_1041_178_638985770347414844 = Quaternion.identity; Vector3 vMinY = Handles.FreeMoveHandle(m_tilemap.transform.TransformPoint(new Vector2(m_tilemap.MapBounds.center.x, m_tilemap.MinGridY * m_tilemap.CellSize.y)), 0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero, EditorCompatibilityUtils.CubeCap);
                var fmh_1042_185_638985770347419926 = Quaternion.identity; Vector3 vMaxY = Handles.FreeMoveHandle(m_tilemap.transform.TransformPoint(new Vector2(m_tilemap.MapBounds.center.x, (m_tilemap.MaxGridY + 1f) * m_tilemap.CellSize.y)), 0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero, EditorCompatibilityUtils.CubeCap);
            ...
            if (EditorWindow.mouseOverWindow == SceneView.currentDrawingSceneView)
            {
                ...
                if (tile != null)
                {
                    ...
                    for (int i = 0; i < handlesPos.Count; ++i)
                    {
                        ...
                        {
                            var fmh_1140_75_638985770347427591 = Quaternion.identity; handlePos = Handles.FreeMoveHandle(handlePos, 0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero, EditorCompatibilityUtils.CubeCap);
